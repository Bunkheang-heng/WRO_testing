...
TARGET_DISTANCE = 0.5  # Example: 50cm from wall
...
try:
    while True:
        ...
        left_error = left_distance - TARGET_DISTANCE
        right_error = right_distance - TARGET_DISTANCE

        # Print distances and errors
        print(f"Left wall: {left_distance:.2f} m (Error: {left_error:.2f} m) | "
              f"Right wall: {right_distance:.2f} m (Error: {right_error:.2f} m)")
        ...
        # Display target distance text on screen
        cv2.putText(depth_colormap, f"Target: {TARGET_DISTANCE:.2f} m", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

        # Show the depth image with markers
        cv2.imshow('Depth Frame with Wall Errors', depth_colormap)
